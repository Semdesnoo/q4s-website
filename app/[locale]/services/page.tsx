...
export default function ServicesPage() {
  ...
  const methods = tn.raw("methods.items") as Array<{ code: string; title: string; desc: string }>;
  const expertise = tn.raw("expertise.items") as string[];
  const sectors = tn.raw("sectors.items") as string[];
    ...
    <>
      ...
      <section id="ndt" className="bg-white pt-12 pb-4 border-t border-gray-100 scroll-mt-[68px]">
        <div className="max-w-[1280px] mx-auto px-6">
          ...
          <h2 className="text-[clamp(32px,4.5vw,64px)] font-black leading-[0.95] tracking-[-0.04em] text-[#000000] mb-6 max-w-3xl">
            {tn("hero.title")}
          </h2>
          <p className="text-[clamp(16px,2vw,22px)] text-gray-600 leading-relaxed max-w-3xl">
            {tn("intro")}
          </p>
        </div>
      </section>
      ...
      <section className="bg-white py-12 lg:py-16">
        <div className="max-w-[1280px] mx-auto px-6">
          <p className="text-[11px] font-semibold uppercase tracking-[0.25em] text-[#e8430a] mb-12">
            {tn("methods.title")}
          </p>
      ...
      {/* ─── NDT expertise + sectors ─── */}
      <section className="bg-[#000000] text-white py-20 lg:py-28">
        <div className="max-w-[1280px] mx-auto px-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
            {/* Expertise */}
            <div>
              <p className="text-sm font-black uppercase tracking-[0.25em] text-[#e8430a] mb-10">
                {tn("expertise.title")}
              </p>
              ...
              <div className="flex flex-col gap-2">
                {sectors.map((s, i) => {
                  ...
                  return (
                    <div
                      key={s}
                      className="flex items-center gap-4 px-5 py-4 border border-white/15 hover:border-[#e8430a] hover:bg-white/5 transition-all duration-200 group"
                    >
